...
public final class PokemonBattleWinListener {
	...
	{
		String[] candidates = {
			"BATTLE_VICTORY_POST", "BATTLE_VICTORY_PRE", "BATTLE_VICTORY", "BATTLE_WON_POST", "BATTLE_WON_PRE", "BATTLE_WON",
			"BATTLE_WIN_POST", "BATTLE_WIN_PRE", "BATTLE_WIN", "BATTLE_ENDED_POST", "BATTLE_ENDED_PRE", "BATTLE_ENDED", "BATTLE_END_POST", "BATTLE_END_PRE",
			"BATTLE_END", "BATTLE_FINISHED_POST", "BATTLE_FINISHED_PRE", "BATTLE_FINISHED", "BATTLE_COMPLETED_POST", "BATTLE_COMPLETED_PRE", "BATTLE_COMPLETED"
		};
	...
	{
		String[] holders = {
			"com.cobblemon.mod.common.api.events.CobblemonEvents", "com.cobblemon.mod.common.api.events.battle.BattleEvents",
			"com.cobblemon.mod.common.api.events.battle.BattleEventHandler", "com.cobblemon.mod.common.api.events.battle.BattleEventHandlers"
		};
	...
	private static boolean tryHookEvent(String eventHolderClassName, String eventFieldName)
	{
		try {
			...
			Object listener = Proxy.newProxyInstance(listenerType.getClassLoader(), new Class<?>[]{listenerType}, new BattleWinHandler());
...
//				player.sendMessage(Text.literal("§a+ " + formatMoney(reward) + " ₽ §7(victoire combat)" + levelSuffix(level)), true);
	...
	private static ServerPlayerEntity resolvePlayer(Object event)
	{
		...
		if (uuid == null)
			uuid = readUuid(callNoArg(event, "getWinnerUuid"));
		if (uuid == null)
			uuid = readUuid(callNoArg(event, "getWinningPlayerUuid"));
		if (uuid == null && battle != null)
			uuid = readUuid(callNoArg(battle, "getWinningPlayerUuid"));
		if (uuid != null) {
	...
	private static ServerPlayerEntity firstPlayerFromIterable(Object maybeIterable)
	{
		...
		if (maybeIterable instanceof Iterable<?> it) {
			for (Object o : it) {
				if (o instanceof ServerPlayerEntity sp)
					return sp;
