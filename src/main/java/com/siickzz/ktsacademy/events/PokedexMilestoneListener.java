...
//		);
	...
	private static Object tryGetPokedexManagerDirect(ServerPlayerEntity player)
	{
		...
		try {
			...
			Object pdm = firstNonNull(callNoArg(cobblemon, "getPlayerDataManager"), callNoArg(cobblemon, "playerDataManager"), getStaticMember(cobblemonCls, "playerDataManager"));

			return firstNonNull(callOneArg(pdm, "getPokedexData", player), callOneArg(pdm, "getPokedex", player), callOneArg(pdm, "getPokedexManager", player));
		} catch (Throwable ignored) {
			return null;
		}
	}
	...
	private static Object tryGetPokedexManagerDirect(UUID uuid)
	{
		...
		try {
			...
			return firstNonNull(callOneArg(pdm, "getPokedexData", uuid), callOneArg(pdm, "getPokedex", uuid), callOneArg(pdm, "getPokedexManager", uuid));
		} catch (Throwable ignored) {
			return null;
		}
	}
	...
	private static int computeCaughtCountFromManager(Object manager)
	{
		...
		Object speciesRecords = firstNonNull(callNoArg(manager, "getSpeciesRecords"), callNoArg(manager, "speciesRecords"), callNoArg(manager, "getRecords"), callNoArg(manager, "getEntries"));
		Iterable<?> values;
		...
		try {
			...
			if (pokedexKey != null) {
				manager = firstNonNull(
					callOneArg(playerData, "get", pokedexKey), callOneArg(playerData, "getOrCreate", pokedexKey), callOneArg(playerData, "getOrMake", pokedexKey),
					callOneArg(playerData, "getInstanced", pokedexKey), callOneArg(playerData, "getInstancedData", pokedexKey), callOneArg(playerData, "getData", pokedexKey)
				);
				if (manager != null)
					return manager;
			}
	...
	private static int extractCaughtCount(Object maybeCount)
	{
		...
		Object v = firstNonNull(
			callNoArg(maybeCount, "getValue"), callNoArg(maybeCount, "value"), callNoArg(maybeCount, "getCount"),
			callNoArg(maybeCount, "count"), callNoArg(maybeCount, "get" )
		);
	...
	private static Object tryGetCobblemonPlayerData(ServerPlayerEntity player)
	{
		...
		Object data = firstNonNull(
			callOneArg(cobblemon, "getPlayerData", player), callOneArg(cobblemon, "getPlayerData", player.getUuid()),
			callOneArg(cobblemon, "getPlayerData", player.getUuid().toString()), callOneArg(cobblemon, "playerData", player), callOneArg(cobblemon, "playerData", player.getUuid())
		);
		if (data != null)
			return data;

		Object storage = firstNonNull(
			callNoArg(cobblemon, "getStorage"), callNoArg(cobblemon, "storage"), callNoArg(cobblemon, "getPlayerDataStore"),
			callNoArg(cobblemon, "playerDataStore"), callNoArg(cobblemon, "getPlayerDataManager"), callNoArg(cobblemon, "playerDataManager")
		);
		data = firstNonNull(
			callOneArg(storage, "get", player), callOneArg(storage, "get", player.getUuid()), callOneArg(storage, "get", player.getUuid().toString()),
			callOneArg(storage, "getPlayerData", player), callOneArg(storage, "getPlayerData", player.getUuid())
		);
		if (data != null)
			return data;

		String[] staticStores = { "com.cobblemon.mod.common.api.storage.player.PlayerDataStore", "com.cobblemon.mod.common.api.storage.player.PlayerDataStoreKt", "com.cobblemon.mod.common.api.storage.player.PlayerDataRegistry"};
		for (String clsName : staticStores) {
			try {
				Class<?> cls = Class.forName(clsName);
				Object inst = getStaticMember(cls, "INSTANCE");
				data = firstNonNull(
					callOneArg(inst, "get", player), callOneArg(inst, "get", player.getUuid()),
					callOneArg(inst, "getPlayerData", player), callOneArg(inst, "getPlayerData", player.getUuid())
				);
				if (data != null)
					return data;
			} catch (Throwable ignored) {}
	...
	private static Object tryGetCobblemonPlayerData(UUID uuid)
	{
		...
		Object data = firstNonNull(
			callOneArg(cobblemon, "getPlayerData", uuid), callOneArg(cobblemon, "getPlayerData", uuid.toString()),
			callOneArg(cobblemon, "playerData", uuid), callOneArg(cobblemon, "playerData", uuid.toString())
		);
		if (data != null)
			return data;
		...
		data = firstNonNull(
			callOneArg(storage, "get", uuid), callOneArg(storage, "get", uuid.toString()),
			callOneArg(storage, "getPlayerData", uuid), callOneArg(storage, "getPlayerData", uuid.toString())
		);
		if (data != null)
			return data;
		...
		for (String clsName : staticStores) {
			try {
				...
				Object inst = getStaticMember(cls, "INSTANCE");
				data = firstNonNull(
					callOneArg(inst, "get", uuid), callOneArg(inst, "get", uuid.toString()),
					callOneArg(inst, "getPlayerData", uuid), callOneArg(inst, "getPlayerData", uuid.toString())
				);
	...
	private static boolean isCaught(Object record, Object caughtEnum)
	{
		...
		if (caughtEnum != null) {
			...
			if (hasAtLeast instanceof Boolean b)
				return b;
		}
