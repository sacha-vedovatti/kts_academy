...
public final class PokemonFishingListener {
	...
	private static boolean tryHookEvent(String eventHolderClassName, String eventFieldName)
	{
		try {
			...
			Object listener = Proxy.newProxyInstance(listenerType.getClassLoader(), new Class<?>[]{listenerType}, new FishingHandler());
	...
	private static boolean shouldCountFishingEvent(Object event, ServerPlayerEntity player)
	{
		...
		if (!bobberHasPokemonHooked(bobber, player))
			return false;
		markCounted(player);
		return true;
	}
	...
	private static boolean bobberHasPokemonHooked(Object bobber, ServerPlayerEntity player)
	{
		...
		if (!cls.contains("PokeRodFishingBobberEntity"))
			return false;

		Object plannedSpawn = firstNonNull(callNoArg(bobber, "getPlannedSpawnAction"), getFieldValue(bobber, "plannedSpawnAction"));
		if (plannedSpawn != null)
			return true;

		Object typeCaught = firstNonNull(callNoArg(bobber, "getTypeCaught"), getFieldValue(bobber, "typeCaught"));
		if (typeCaught != null) {
			String s = typeCaught.toString().toUpperCase(Locale.ROOT);
			if (s.contains("POKEMON"))
				return true;
		}

		Object hooked = firstNonNull(callNoArg(bobber, "getHookedEntity"), getFieldValue(bobber, "hookedEntity"));
		if (hooked instanceof Entity e) {
			if (e.getClass().getName().contains("PokemonEntity"))
				return true;
		}
	...
	{
		Object directBobber = firstNonNull(callNoArg(event, "getBobber"), getFieldValue(event, "bobber"));
		if (directBobber != null)
			return directBobber;

		Object ctx = callNoArg(event, "getContext");
		Object fromCtx = firstNonNull(callNoArg(ctx, "getBobber"), callNoArg(ctx, "bobber"), getFieldValue(ctx, "bobber"));
		if (fromCtx != null)
			return fromCtx;
	...
	{
		Object bobber = firstNonNull(getFieldValue(player, "fishHook"), getFieldValue(player, "fishingBobber"), callNoArg(player, "getFishHook"), callNoArg(player, "getFishingBobber"));
		if (bobber != null)
			return bobber;
		...
		while (c != null && c != Object.class) {
			try {
				for (Field f : c.getDeclaredFields()) {
					...
					if (v instanceof FishingBobberEntity)
						return v;
				}
			} catch (Throwable ignored) {}
			c = c.getSuperclass();
	...
	private static ServerPlayerEntity resolvePlayer(Object event)
	{
		...
		if (direct instanceof ServerPlayerEntity sp)
			return sp;

		Object bobber = callNoArg(event, "getBobber");
		if (bobber != null) {
			Object owner = firstNonNull(
				callNoArg(bobber, "getPlayerOwner"), callNoArg(bobber, "getOwner"), callNoArg(bobber, "getOwnerEntity"),
				callNoArg(bobber, "getPlayer"), callNoArg(bobber, "getThrower"), callNoArg(bobber, "getAngler"),
				getFieldValue(bobber, "player"), getFieldValue(bobber, "owner"), getFieldValue(bobber, "playerEntity"), getFieldValue(bobber, "thrower"), getFieldValue(bobber, "bobberOwner")
			);

			if (owner instanceof ServerPlayerEntity sp)
				return sp;
			if (owner instanceof Entity e && e instanceof ServerPlayerEntity sp2)
				return sp2;
		...
		Object fromCtx = firstNonNull(callNoArg(context, "getPlayer"), callNoArg(context, "player"), getFieldValue(context, "player"));
		if (fromCtx instanceof ServerPlayerEntity sp3)
			return sp3;
		return null;
	}
