...
public final class PokemonDefeatListener {
	...
	private static void onKill(ServerWorld world, Entity attacker, LivingEntity killed)
	{
		...
		player.sendMessage(Text.literal("§a+ " + formatMoney(reward) + " ₽ §7pour avoir vaincu §f" + safeName(killedPokemon, killed) + levelSuffix(level)), false);
	}
	...
	private static ServerPlayerEntity resolvePlayer(MinecraftServer server, Entity attacker)
	{
		...
		if (ownerUuid == null)
			ownerUuid = readUuid(callNoArg(attacker, "getOwnerUUID"));
		if (ownerUuid == null)
			ownerUuid = readUuid(callNoArg(attacker, "getOwnerId"));
		if (ownerUuid != null)
